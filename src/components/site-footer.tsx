...
/** 页脚参考区：片单 / 知识等并列摘要块。 */
function FooterReference({ id, title, href, linkLabel, items }: FooterReferenceProps) {
  return (
    <section aria-labelledby={id}>
      ...
      <dl className="mt-4 space-y-4">
        {items.map((item) => (
          <div key={item.label}>
            <dt className="text-xs font-medium text-[var(--sz-text-soft)]">{item.label}</dt>
            <dd className="mt-1 text-pretty text-xs leading-5 text-[var(--sz-muted)]">{item.note}</dd>
          </div>
        ))}
      </dl>
    </section>
    ...
    <footer className="relative z-10 mx-auto max-w-7xl px-6 pb-[max(2.5rem,env(safe-area-inset-bottom,0px))] md:px-10">
      <div className="border-t border-[color:var(--sz-border)] pt-10 md:pt-12">
        <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
          ...
          <nav aria-label="页脚导航" className="flex flex-wrap gap-x-5 gap-y-2 text-sm md:justify-end">
            {navItems.map((item) => (
        ...
        <div className="mt-10 grid gap-10 md:mt-12 md:grid-cols-2 md:gap-12 lg:gap-16">
          <FooterReference id="footer-curation" title="片单" href="/collections" linkLabel="全部片单" items={curationHints} />
          <FooterReference id="footer-knowledge" title="高清知识" href="/knowledge" linkLabel="知识库" items={knowledgeItems} />
        </div>
